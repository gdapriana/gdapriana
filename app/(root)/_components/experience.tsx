    ...
    <main className="md:p-20 p-8 flex gap-20 flex-col justify-start items-stretch">
      <div className="overflow-hidden flex justify-start items-center">
        <m.h1 ref={headerRef} className="md:text-5xl py-12 text-2xl font-bold text-muted-foreground w-[80%]">
          I have <m.span animate={headerInView ? {color: "hsl(var(--primary-foreground))"} : {color: ""}} transition={{delay: 0.5, duration: 1, ease: "anticipate"}}>
          {(new Date().getFullYear()) - 2023} years of experience</m.span> running successful design practices together with startups and top brands.
        </m.h1>
      </div>
      ...
      <div className="flex flex-col-reverse gap-12 lg:flex-row justify-start items-stretch lg:justify-center lg:items-end">
        <div className="lg:w-1/3 w-full flex justify-center items-end gap-4">
          <Link className="py-4 px-7 w-full lg:w-auto md:py-6 md:px-10 border border-stone-800 text-primary-foreground" href={contact.cv}>Download CV</Link>
          <Link className="py-4 px-7 w-full lg:w-auto md:py-6 md:px-10 border border-stone-800 text-primary-foreground" href={contact.linkedin}>Connect Linkedin</Link>
        </div>
